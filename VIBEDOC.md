...
- `enchanting-table-max-level` 只属于 `vanilla/<原版附魔>.yml` 原版附魔覆盖配置，不属于自定义附魔配置；不要写进 `enchantments/...` 文件。
- `obtain.villager-trade: false` 时，`villager-trade-price-range` 会被插件忽略；如果村民交易开启或未显式关闭，价格区间必须是两个整数。
- 数值可以用 `{level}` 表示当前附魔等级。
- `cooldown`、`cooldown-levels`、`cooldown-formula`、`duration` 等时间单位是 tick，20 tick = 1 秒。
- 需要不同等级不同冷却时，优先使用 `cooldown-levels`；需要规则化缩放时使用 `cooldown-formula`。实际优先级是：当前等级命中 `cooldown-levels` > `cooldown-formula` > 固定 `cooldown`。
...
原版附魔覆盖配置放在 `vanilla/<原版附魔id>.yml`，只用于调整 Minecraft 原版附魔，例如 `sharpness.yml`、`mending.yml`、`lunge.yml`。

AI 只有在用户明确要求修改原版附魔、原版附魔等级、原版附魔禁用、原版附魔冲突或原版附魔适用物品时，才输出 `vanilla/...` 配置。不要把下面这些字段写入自定义附魔的 `enchantments/...` 文件。
...
- `disabled`: 是否禁用这个原版附魔。
- `max-level`: 这个原版附魔允许存在的总上限。`-1` 表示使用原版默认上限。铁砧、指令、高等级附魔书等保留等级时看这个值。
- `enchanting-table-max-level`: 附魔台最高可生成等级。`-1` 表示使用原版附魔台默认上限。该值不会超过 `max-level`。
- `conflicts`: 额外冲突附魔 ID 列表。
- `applicable-items`: 允许该原版附魔应用的物品分类或材料。
...
含义是：铁砧、指令或附魔书可以保留锋利 X，但附魔台最多只能生成锋利 V。
...
- `trigger`：什么时候触发。
- `cooldown`：固定冷却 tick，可选但推荐写。
- `cooldown-levels`：按附魔等级指定冷却 tick，例如 `1: 100`、`3: 60`；只对命中的等级生效。
- `cooldown-formula`：按公式计算冷却 tick，例如 `"{level} * 20"`；仅在当前等级没有命中 `cooldown-levels` 时使用。
...
效果块冷却也可以用于描述：`{cooldown}` 显示当前等级实际 tick，`{cooldown_seconds}` 显示当前等级实际秒数；如果配置了 `cooldown-levels` 或 `cooldown-formula`，占位符会按当前等级解析后的冷却显示。动作或条件里的额外参数也可以用同名占位符读取，例如 `range: "{level} + 3"` 可以在描述里写 `{range}`。带连字符的参数同时支持下划线写法，例如 `{max-blocks}` 和 `{max_blocks}`、`{cooldown-seconds}` 和 `{cooldown_seconds}` 都可以。未知占位符会原样显示，所以 AI 不要写本文没定义、配置里也不存在的占位符。
...
src/main/resources/vanilla/        原版附魔覆盖配置
src/main/resources/lang/           多语言文本
src/main/resources/gui/            GUI 布局配置
src/main/resources/items/          自定义物品配置
```

铁砧突破石和“过于昂贵”突破 GUI 属于系统道具/系统 GUI 配置，不是附魔 YAML 字段。它们分别由 `items/custom-items.yml` 中的 `anvil-breakthrough-stone` 和 `gui/anvil-breakthrough.yml` 控制。除非用户明确要求修改系统道具或 GUI，不要在附魔配置里编造 `breakthrough`、`too-expensive`、`anvil-gui` 之类字段。
...
18. 如果输出 `enchanting-table-max-level`，是否确认目标文件是 `vanilla/...` 原版附魔覆盖配置，而不是 `enchantments/...` 自定义附魔配置。
19. 输出内容是否只围绕附魔配置和语言条目，没有混入源码修改或无关工程流程。
