...
## 当前版本必须注意的附魔书 lore 规则

附魔书和已附魔物品上的说明不是随便复制 `description` 原文，而是由插件根据 `effects` 自动生成。AI 写附魔时必须让 `effects` 里的参数足够明确，这样插件才能把 `{level}`、概率、持续时间、等级、范围、倍数等占位符计算成玩家看到的数值。

推荐语言描述仍然要写，但它主要用于图鉴、命令信息或兜底说明：
...
effects:
  - trigger: TAKE_DAMAGE
    ...
    actions:
      - type: HEAL
        value: "{level} * 0.075 + 0.025"
      - type: ADD_POTION_SELF
        extra-params:
          potion: DAMAGE_RESISTANCE
          amplifier: "floor(({level} - 1) / 4)"
...
```text
韧性 V
    - 5级：有 18% 概率恢复 40% 生命并获得抗性提升 2 级，持续 8 秒。
```
...
| 效果 | 推荐参数 | 说明 |
| --- | --- | --- |
| `HEAL` | `value` | 小于等于 1 时按最大生命百分比理解，例如 `0.4` 会显示为 40% 生命。 |
| `ADD_POTION_SELF` / `ADD_POTION_TARGET` | `potion`、`amplifier`、`duration` | `duration` 是 tick，显示时会转成秒；`amplifier: 0` 表示 1 级药水。 |
| `BONUS_DROP` | `multiplier` | 掉落倍数必须写 `multiplier`，不要只写 `value`，否则旧模板容易显示不清楚。 |
...
src/bootstrap/                     Paper 版本启动适配
```

版本兼容目录不要再使用 `compat-paper-1.21.1` 这类临时命名。当前约定是：

```text
src/bootstrap/shared/
src/bootstrap/entrypoint/
src/bootstrap/paper/v1_21_R1/
src/bootstrap/paper/v1_21_R6/
```

AI 如果只是在写附魔配置，通常不需要修改 `src/main/java` 或 `src/bootstrap`。

## Release 与更新检测规则

插件的自动更新检测读取 GitHub Release。发布正式版本时，Release tag 应使用：

```text
v1.0.2
```

不要只推送代码不创建 Release，否则服务器上的更新检测无法发现新版本。Release 附件应上传最终插件 jar：

```text
FotiaEnchantment-版本号.jar
```
...
7. 每个效果块是否有 `trigger`。
8. 每个效果块是否有至少一个 `actions`。
9. 触发器、条件、效果是否都来自本文列表。
10. 概率、伤害、范围、冷却是否合理。
11. 是否同时写了中文和英文语言条目。
12. 描述是否是玩家能看懂的实际效果。
13. 如果附魔有概率，是否在 `conditions` 里写了 `chance`，而不是只写在文字描述里。
14. 附魔书 lore 需要显示的数值，是否都能从 `effects` 参数和 `{level}` 公式计算出来。
15. 掉落倍数是否使用 `multiplier`，连锁挖掘数量是否使用 `max-blocks`。
16. 如果要写入仓库，是否把 `VIBEDOC.md`、语言文件和默认资源都放在 Git 仓库根目录内。
