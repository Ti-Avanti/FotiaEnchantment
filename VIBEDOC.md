...
`description` 支持的常用占位符：

| 占位符 | 来源 |
| --- | --- |
| `{level}` | 当前附魔等级。 |
| `{chance}` | 当前等级下第一个 `chance` 条件的百分比。 |
| `{value}` | 当前等级下第一个动作的 `value`。 |
| `{amount}` | 伤害、治疗、修复等动作的数量；没有单独 `amount` 时通常来自动作 `value`。 |
| `{damage}` | 伤害类动作的数值。 |
| `{percent}` | 减伤、吸血、反伤等百分比动作的数值。 |
| `{multiplier}` | 倍率参数，例如掉落倍数或伤害倍率。 |
| `{radius}` | `radius` 参数；如果是连锁挖掘，也可来自 `max-blocks`。 |
| `{range}` | `range` 参数。 |
| `{duration}` | `duration` 参数原始 tick 数。 |
| `{seconds}` | `duration / 20` 后的秒数。 |
| `{power}` | `power` 参数或击退/发射强度。 |
